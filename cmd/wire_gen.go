...
func InitializeWsServer(debug bool, dbConf *data.Database) (*gin.Engine, error) {
	...
	platformService := platform2.NewPlatformService(platformRepo, loginService, videoService, userService, messageService, encryptEncrypt)
	...
	engine := server.NewWebSocketServer(controllerPlatform, arith, controllerMessage, controllerRegister)
	return engine, nil
}
