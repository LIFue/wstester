...
func NewWsUtil(url string, keepAlive bool) (*WsUtil, error) {
	...
	w.SetKeepAliveMsg(model.NewWsMsg(code.MSG_KEEP_ALIVE, model.NewKeepAlive(60, time.Now().UTC().String())))
...
func (w *WsUtil) SendMsg(m interface{}) (int64, error) {
	// msg, ok := m.(model.WsMsg)
	// if !ok {
